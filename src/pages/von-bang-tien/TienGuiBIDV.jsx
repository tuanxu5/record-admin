...
const TienGuiBIDVPage = () => {
  ...
  // Hàm nhóm dữ liệu ps_no và ps_co theo period
  const groupDataByPeriod = useMemo(() => {
    ...
    const psNo = sortedKeys.map(key => grouped[key].psNo);
    const psCo = sortedKeys.map(key => grouped[key].psCo);
  ...
  // Cấu hình ApexCharts cho ps_no và ps_co (biểu đồ nhóm)
